...
export const useAdminOrderList = ({ archived = false }) => {
    ...
    return useQuery<Tables<'orders'>[]>({
        ...
        queryFn: async () => {
          const { data, error } = await supabase.from('orders').select('*').in("status", stats).order('id', {ascending: false});
          if (error) {
...
export const useInsertOrder = () => {
    ...
    return useMutation({
      async mutationFn(data: InsertTables<'orders'>) {
         const {error, data: newProduct} = await supabase.from('orders').insert({
            ...data, user_id: userId!
        }).select().single();
...
export const useUpdateOrder = () => {
  ...
  return useMutation({
    async mutationFn({id, updatedFields}: {updatedFields: UpdateTables<'orders'>, id: number}) {
